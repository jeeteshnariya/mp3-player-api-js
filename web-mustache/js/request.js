 ...
 */
function Request() {
    this.templatePath = 'http://localhost/web-mustache/template/';
    this.templateExt  = '.mustache';
    this.apiUrl       = 'http://localhost/api/';
}

/**
 * Execute une requête pour obtenir un template et retourne les données HTML avec le callback passé en paramètre
 * @param name
 * @param nav
 * @param data
 * @param callback
...
Request.prototype.load = function(name, nav, data, callback) {
    $.get(this.templatePath + name + this.templateExt, function(response) {
        var tpl = Mustache.render(response, data);
