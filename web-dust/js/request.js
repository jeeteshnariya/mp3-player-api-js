 ...
 */
function Request() {
    this.templatePath = 'http://localhost/web-dust/template/';
    this.templateExt  = '.dust';
    this.apiUrl       = 'http://localhost/api/';
}

/**
 * Execute une requête pour obtenir un template et retourne le callback en passant les données HTML
 * @param name
 * @param nav
 * @param data
 * @param callback
...
Request.prototype.load = function(name, nav, data, callback) {
    $.get(this.templatePath + name + this.templateExt, function(response) {
        var compiled = dust.compile(response, 'templateRequested');

        dust.loadSource(compiled);

        dust.render('templateRequested', data, function(err, out) {
            if (nav !== null) {
